...
module.exports = async (req, res, next) => {
  try {
    const { take, skip, companyId } = req.query;
    let query = 'SELECT Usuario.id, Usuario.nome, email, permite_foto, Empresa.nome AS nome_empresa, COUNT(*) OVER() as Total FROM banco01.dbo.usuario Usuario ' +
      'LEFT JOIN banco01.dbo.empresa Empresa ON Empresa.id = Usuario.id_empresa ';

    if (companyId !== '0' && companyId !== 'undefined') {
      query += 'WHERE Usuario.id_empresa = ' + companyId;
    }
